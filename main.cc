...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (b_mode) {
            cnt_cmd++;
            if (p == NULL) { // Empty line in batch input file
                continue;
            }
            if (strcmp(p, ";;") != 0) {
	      // cout << "-------------------------------" << endl;
	  ...
	  command = token_cmd[0];
	  if ((command[0] == ';') && (command[1] == ';')) { // for writing comments in batch input file
	    continue;
	  }
	  else {
		...
		if(str1[i] == '$') {
		  ...
		  for (itr = macro_defs_in_batch_inp.begin();
		    ...
		    str2= itr->first;
		    N=str2.length();
		    if (M-i < N) continue; // No match possible
		    for(j=0;j<N;j++) {
		      if(str1[i+j] == str2[j]) {
			if(j==(N-1)) {
			  macro_found = true;
        ...
        if (command == "start") { // Hack to make the earlier way of invoking graph manager work
...
//            cout << "PIK3R5 asserted absent" << endl;
            ...
            while (1) {
                ...
                else if (soln == "sat") { // got a sat solution
                    soln_counter++;
...
//                exit(1);
                ...
                string timefile = "limits_timefile.txt";
