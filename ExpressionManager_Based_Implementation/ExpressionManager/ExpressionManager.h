...
private:
    ...
    struct updateData {
        ...
        updateData(t_Expression* idx, t_Expression* val, int lev, TypeOfExpressionTuple type) :
        index(idx), value(val), level(lev), type_info(type) {
        }
...
public:
     ...
     * 
     */
    pair< vector<t_MemoryBounds>, int> getBoundsOfArray(t_Expression* array_expr){ int a; return getBoundsOfArray(array_expr, a);}
...
public:
    ...
    inline bool setExpressionEvaluatorWithMemoryEntries(vector<t_MemoryArrayBoundsInfo*> &MemoryEntries) {
        this->m_evaluator->m_MemoryArraysForEvaluation = MemoryEntries;
    }
...
public:
    ...
    enum type_of_operator {
        UNKNOWN, LOGICAL, RELATIONAL, BITWISE
    };
